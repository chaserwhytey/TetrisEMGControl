...
if nargout
    ...
    % Callback for the 'Start' ('Pause', 'Continue') button.
        switch get(S.pbt,'string')
            case 'Start'
                ...
                DELTA_T = 0.1;
                RUN_LENGTH  = 10;
                %floor(1*60*10/DELTA_T); %time to run loop in samples
                ...
                MATLAB_DIR = 'C:\Users\Kenny Lu\Documents\Matlab'; %FILL IN
                addpath(genpath(MATLAB_DIR)) %add those tools to the path
                LOOP_TIME_FF = 0.001; %'fudge factor' for loop time 

                emg_inputs   = zeros(NUM_CHANNELS,RUN_LENGTH); %FILL IN. streamed emg inputs, [# channels x time you will run your loop]
                elapsed_time = nan(RUN_LENGTH, 1); %FILL IN. time it takes loop to execute [time you will run your loop x 1]
                emg_plot     = zeros(NUM_CHANNELS, PLOT_LENGTH); %FILL IN: running buffer for data to plot, [# channels x time points you will plot]
                ...
                for i= 1:RUN_LENGTH
                    ...
                    emg_inputs(:,i) = mean(abs(data_chunk), 2);
                    emg_plot  = circshift(emg_plot, -1, 2);
                    emg_plot(:,end) = emg_inputs(:,i);
                    plot(ax_emg, time_plot, emg_plot, 'linewidth', 2.5) %FILL IN []s with appropriate arguments to pass in
                    histogram(ax_hist, elapsed_time, HIST_BINS, 'Normalization', 'probability'); %FILL IN []s with appropriate arguments to pass in
                    drawnow %this command forces matlab to push updates to the figure NOW (rather than after our code finishes running)
                    pause((DELTA_T - LOOP_TIME_FF) - etime(clock,t0)); %FILL IN
                    elapsed_time(i) = etime(clock,t0)*1000;
                end
                %%
    ...
    function [] = game_step(varargin, emg_inputs)
        %%%%%% done initializing socket. 
         [left, right, turn] = emg_control(emg_inputs);
         run_emg_control();
